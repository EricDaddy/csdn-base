...
public class MenuServiceImpl extends ServiceImpl<MenuMapper, Menu> implements IMenuService {
    ...
    public boolean submit(Menu menu) {
        ...
        Integer cnt = baseMapper.selectCount(ObjectUtil.isEmpty(menu.getId())? queryWrapper : queryWrapper.notIn("id",menu.getId()));
        if(cnt > 0) {
            throw new ApiException("当前菜单编码或权限标识已存在已存在");
        }
